...
def cat_msg2cluster_group(x_groups,msg_tokens):
    x_groups_cated = []
    for x in x_groups: 
        x = x.unsqueeze(dim=0) #torch.Size([1, 1, 2449, 512])
        try:
...
class basedBasicLayer(nn.Module):
    ...
    def forward(self,data,mask_ratio=0):
        # x_groups = data
        data = self.GroupsAttenLayer(data,mask_ratio) 
        # data = (x_groups, msg_tokens_num)
...
class BasedMILTransformer(nn.Module):
    ...
    def forward(self,FusionHisF,x,memory=False,coords=False,mask_ratio=0):
        ...
        # trandata_ppo = (x_groups, self.msg_tokens_num) #
        for i in range(len(self.basedlayers)): # 
            x_groups = self.basedlayers[i](x_groups,mask_ratio=0)
        
        #---> head   
        # x_groups, _ = trandata_ppo #  
        msg_token = x_groups[0][:,:,0,:]

  
        memory.msg_states.append(x_groups[0][:,:,0,:]) #  pseudo-bag representation
        memory.merge_msg_states.append(x_groups[0][:,:,0,:]) #  cross pseudo-bag representation
        
         
        FusionHisF.SFFR(msg_token,memory) # CPF

        
        msg_token = memory.merge_msg_states[-1].view(1,self.args.embed_dim)

        results_dict = self.head(msg_token) #pseudo-
